...
const defaultVaultPath = '/Users/nickgeorge-studio/Documents/nicks-mind-map'
const defaultBrowserHarnessPath = '/Users/nickgeorge-studio/Projects/browser-harness/.venv/bin/browser-harness'
...
Options:
  --date <YYYY-MM-DD>          Edition date. Defaults to today's local date.
  --vault <path>               Obsidian vault path. Defaults to DFE_VAULT_PATH, OBSIDIAN_VAULT_PATH, or ${defaultVaultPath}.
  ...
  --model <model>              OpenAI text/vision model. Defaults to OPENAI_MODEL or gpt-5.5.
  --image-model <model>        OpenAI image model. Defaults to OPENAI_IMAGE_MODEL or gpt-image-2.
  --browser-harness <path>     browser-harness executable. Defaults to ${defaultBrowserHarnessPath}.
...
function parseArgs(argv) {
  const options = {
    mode: 'from-scratch',
    date: localDate(),
    vault: process.env.DFE_VAULT_PATH || process.env.OBSIDIAN_VAULT_PATH || defaultVaultPath,
    windowDays: defaultSignalWindowDays,
    maxNotes: defaultMaxNotes,
    maxSources: defaultMaxSources,
    model: process.env.OPENAI_MODEL || 'gpt-5.5',
    imageModel: process.env.OPENAI_IMAGE_MODEL || 'gpt-image-2',
    browserHarness: process.env.BROWSER_HARNESS_PATH || defaultBrowserHarnessPath,
    sourceTool: process.env.DFE_SOURCE_TOOL || 'browser-harness',
    imageSize: '1536x1024',
    imageQuality: 'medium',
    publish: false,
    ux: null,
    ...
    remapPlate: false,
    generationName: null,
    promptedMaskDir: process.env.DFE_PROMPTED_MASK_DIR || null,
    skipMask: false,
  }
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--window-days') {
      options.windowDays = Number(readValue(arg))
      continue
    }

    if (arg.startsWith('--window-days=')) {
      options.windowDays = Number(arg.slice('--window-days='.length))
      continue
    }

    if (arg === '--max-notes') {
      options.maxNotes = Number(readValue(arg))
      continue
    }

    if (arg.startsWith('--max-notes=')) {
      options.maxNotes = Number(arg.slice('--max-notes='.length))
      continue
    }

    if (arg === '--max-sources') {
      options.maxSources = Number(readValue(arg))
      continue
    }
    ...
    if (arg === '--image-model') {
      options.imageModel = readValue(arg)
      continue
    }
    ...
    if (arg === '--browser-harness') {
      options.browserHarness = readValue(arg)
      continue
    }

    if (arg.startsWith('--browser-harness=')) {
      options.browserHarness = arg.slice('--browser-harness='.length)
      continue
    }

    if (arg === '--source-tool') {
      options.sourceTool = readValue(arg)
      continue
    }

    if (arg.startsWith('--source-tool=')) {
      options.sourceTool = arg.slice('--source-tool='.length)
      continue
    }

    if (arg === '--image-size') {
      options.imageSize = readValue(arg)
      continue
    }
    ...
    if (arg === '--image-quality') {
      options.imageQuality = readValue(arg)
      continue
    }

    if (arg.startsWith('--image-quality=')) {
      options.imageQuality = arg.slice('--image-quality='.length)
      continue
    }
    ...
    if (arg === '--ux') {
      options.ux = readValue(arg)
      continue
    }

    if (arg.startsWith('--ux=')) {
      options.ux = arg.slice('--ux='.length)
      continue
    }

    if (arg === '--edition') {
      options.editions.push(...splitList(readValue(arg)))
      continue
    }

    if (arg.startsWith('--edition=')) {
      options.editions.push(...splitList(arg.slice('--edition='.length)))
      continue
    }

    if (arg === '--all-editions') {
      options.allEditions = true
      continue
    }

    if (arg === '--remap-plate') {
      options.remapPlate = true
      continue
    }

    if (arg === '--generation-name') {
      options.generationName = readValue(arg)
      continue
    }

    if (arg.startsWith('--generation-name=')) {
      options.generationName = arg.slice('--generation-name='.length)
      continue
    }

    if (arg === '--prompted-mask-dir') {
      options.promptedMaskDir = readValue(arg)
      continue
    }

    if (arg.startsWith('--prompted-mask-dir=')) {
      options.promptedMaskDir = arg.slice('--prompted-mask-dir='.length)
      continue
    }
  ...
  if (options.mode === 'from-scratch' && options.sourceTool === 'browser-harness' && !fsSync.existsSync(options.browserHarness)) {
...
function localDate() {
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: process.env.TZ || 'America/Detroit',
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
  }).format(new Date())
...
async function runFromScratchMode(options) {
  ...
  const internalSteps = [
    {
      name: 'Mine recent Obsidian signals',
      tool: 'Node fs Markdown scanner',
      command: `internal:mine-signals --vault ${JSON.stringify(options.vault)} --window-days ${options.windowDays} --max-notes ${options.maxNotes} --avoid-recent-terms ${JSON.stringify(recentDiversityAvoidTerms.join(','))}`,
      run: async () => {
